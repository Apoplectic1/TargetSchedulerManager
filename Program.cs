...
internal static class Program
{
    ...
    {
        // The verb is positional; accept "writeback" or "--writeback" (dash-tolerant) so a stray leading dash can't
        // silently fall through to a full catalog build.
        if (args.Length > 0 && IsVerb(args[0], "writeback"))
            return await WriteBack(args);

        // --target / --apply only mean anything under the writeback verb. If they appear without it, the user almost
        // certainly meant writeback — say so instead of silently rebuilding the whole catalog (the easy mistake is
        // `--writeback` without the `--` separator under `dotnet run`, so the verb never reaches the app).
