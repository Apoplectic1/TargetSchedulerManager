...
the displayed/editable columns of `target`, `exposureplan`, and `project` (per design D2) — recording per
(table, key, column) the old and new values. A first-ever pull (no local db yet) SHALL record nothing. A
row added on the remote SHALL record one inbound entry; a row deleted on the remote SHALL record nothing.
Columns absent from either snapshot (TS schema drift) SHALL be skipped without error.
...
tooltip SHALL summarize direction counts. Blank marks SHALL show no tooltip.
...
#### Scenario: Header tooltip summarizes
- **WHEN** the user hovers a target header marked `⇄` covering two inbound fields and three unpushed fields
...
Disk-plane leaf rows SHALL never mark (leaf marks key on the plan key only; target/project-level changes
mark the header). Exposure-template edits SHALL mark no row (they remain visible in the sync badge and
push review).
...
#### Scenario: Template edit marks nothing
- **WHEN** the user edits an exposure template used by several plans
- **THEN** no grid row marks, while the sync badge counts the unpushed entry
