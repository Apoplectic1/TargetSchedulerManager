...
- [ ] 1.1 Cross-repo checkpoint: implement against ISM's revised
...
- [ ] 2.1 Envelope constant → `v: 2`.
- [ ] 2.2 `project-upsert` record widens: settings block + `is_mosaic`, full committed values
      from the local working copy; sentinel→null translations per contract v2 table.
- [ ] 2.3 `exposure-template-upsert` mirror widens: moon-relax triplet.
- [ ] 2.4 Observed emission extends to project rows: pull-diff correlates project-table rows
...
- [ ] 2.5 Verify the authored-by-construction premise for project columns: neither TS's
...
- [ ] 3.1 Fixture emission: pushed settings edit emits a project-upsert carrying the full v2
      block; sentinels emit as JSON null (`minimumaltitude 0.0`, `maximumAltitude 0.0`).
- [ ] 3.2 Template mirror carries the relax triplet on every plan-upsert ride-along and on a
      pushed template edit.
- [ ] 3.3 Existing v1 emission suite re-pinned at v2 (envelope + widened ops); no sent-tracking
      state introduced.
- [ ] 3.4 Observed project emission: an inbound project-settings diff emits one full-value
...
- [ ] 4.1 Full TSM suite green, zero warnings;
