# catalog-export — delta for add-inbox-v2-emission

## REMOVED Requirements

### Requirement: Inbox transport per contract v1

**Reason**: Superseded whole by *Inbox transport per contract v2* (ADDED below) — same
transport rules, v2 envelope; the version is in the requirement and scenario names, so this is
a remove-and-replace rather than a modify.

### Requirement: Observed inbound target changes emit at pull

**Reason**: Superseded whole by *Observed inbound target and project changes emit at pull*
(ADDED below) — the same pull-diff pass, scope widened to project-table rows per contract v2
(the "feed-v2 lane" this requirement's own text pointed project rows at).
