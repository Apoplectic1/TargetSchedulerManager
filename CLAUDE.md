...
**not** own or write `Catalog.db`. Since 2026-08-12 every committed push — and every pull, for the
externally-authored changes it observes arriving — **feeds ISM's catalog inbox**
(openspec `catalog-export` — TSM's one ISM-era duty: user-authored intent as JSONL upserts per the ISM inbox
contract, at v2 since 2026-08-13; TSM still never opens `Catalog.db`; dies at TS retirement).
Full statement → `ARCHITECTURE.md` → *Key facts*.
